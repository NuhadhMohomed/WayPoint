...
| **`REQ-TECH-06`** (Agentic AI Subsystem) | `FR-AI-01` to `08` | Multi-Agent AI Subsystem | Level 4 multi-agent orchestration, 10 allow-listed tools, persisted state. | Golden test cases, schema validation tests. | Agentic AI Evaluation Report (5–8 pgs). |
...
| **`REQ-DB-05`** (AI State Persistence) | `FR-AI-01`, `FR-AI-07` | AI Persistence Tables | Tables `AiWorkflow`, `AiWorkflowStep`, `AiToolCall`, `AiValidationResult`. | DB persistence unit tests. | Agentic AI & Database Report. |
| **`REQ-AI-02`** (Planning & Delegation) | `FR-AI-01`, `REQ-MULTI-01` | Planner Agent | Planner agent decomposing objectives into ordered multi-step execution plans. | Plan structure assertion tests. | Agentic AI Evaluation Report. |
| **`REQ-AI-03`** (10 Allow-Listed Tools) | `FR-AI-02` | Tool Runner Service | 10 backend tool wrappers validating DTO inputs and executing business logic. | Tool DTO validation unit tests. | Agentic AI Evaluation Report. |
...
| **`REQ-AI-08`** (Prompt Injection & Safe Failure) | `FR-AI-04`, `FR-AI-08` | AI Resilience & Guardrails | Input sanitization, timeouts, retry caps, safe-failure state fallback. | Prompt injection & error recovery tests. | Agentic AI Evaluation Report. |
...
5. **Agentic AI Orchestration Framework**: Confirm final choice for `ADR-03` (Microsoft Semantic Kernel in C# vs Python LangGraph microservice).
6. **Cloud Hosting Platform Provider**: Confirm final provider choice for `ADR-05` (Render vs Azure App Service / Railway).
